...
void handle_command(const std::string& command) {
	...
	if (arguments.at(0) == "cmds" || arguments.at(0) == "commands") {
		MessageBox(NULL, commands, "Ashpe", MB_OK);
	}
	...
	//	auto_exec();
		...
		print_output(RGB(0, 0, 0), "Omithus v%s Loaded! ", SW_VERSION);
...
static LRESULT CALLBACK dll_window_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_COMMAND:
	{
		...
		case BUTTON_REMEMBERME:
		{
			if (!loadedscr)
			{
				MessageBox(NULL, "You're unable to store your data due to an error with the setting of files. Please re-launch Cyber and then attempt to re-do this", "Script-Ware", MB_ICONERROR | MB_OK);
				break;
			}
		...
		case MENU_EXIT: {
			if (MessageBox(NULL, "Are you sure you want to close Cyber? Roblox will close with it.", "Confirmation", MB_ICONWARNING | MB_YESNO) == IDYES) {
				exit(0);
			}
			break;
		}
		case MENU_DEATTACH:
		{
			if (MessageBox(NULL, "Are you sure you want to close Omithus? This will close Script-Ware but not Roblox", "Confirmation", MB_ICONWARNING | MB_YESNO) == IDYES) {
				ExitThread(0);
			}
			break;
		}
		case MENU_ABOUT:
		{
			MessageBox(NULL, "Omithus | The unrestricted script execution product\n\nWritten and developed by OPDev", "Omithus", MB_ICONINFORMATION | MB_OK);
			break;
		}
		default:
...
void create_toolbar_menu(HMENU hhmenu)
{
	...
	AppendMenu(hhmenu, MF_POPUP, (UINT_PTR)scriptware_menu, TEXT("Cyber"));
...
static void init_gui() {
	...
	/* Login Screen */
	main_login_window = CreateWindowEx(0, "ScriptWareLogin", "Omithus | Unrestricted Script Execution", WS_SYSMENU | WS_MINIMIZEBOX, 100, 100, 316, 293, 0, 0, hinstance, 0);
	...
	HWND label1 = CreateWindow(TEXT("static"), "Omithus | Unrestricted Script Execution - Developed by OPDev", WS_CHILD | WS_VISIBLE | WS_TABSTOP, 10, 245, 300, 190, main_login_window, (HMENU)(501), hinstance, NULL);
	...
	/* Loading Screen */
	main_load_window = CreateWindowEx(0, "ScriptWareLoading", "Omithus | Unrestricted Script Execution", WS_SYSMENU, 100, 100, 316, 97, 0, 0, hinstance, 0);
	...
	/* Main Screen */
	main_window = CreateWindowEx(0, "ScriptWare", "Omithus | Unrestricted Script Execution", WS_SYSMENU | WS_MINIMIZEBOX, 100, 100, 397, 362, 0, window_menu, hinstance, 0);
...
void initialize() {
	...
	if (debug_mode) {
		output_label("Omithus: Beginning Scan... ");
	}
	...
	if (debug_mode)
		output_label("Omithus: Executing scripts held inside data->autoexec");

	if (debug_mode)
		output_label("Omithus: Init script loading");
	...
	print_output(RGB(0, 0, 0), "Cyber v%s Loaded!\n", SW_VERSION);

	if (auth_firsttime) {
		if (MessageBox(NULL, "Would you like to view the Script-Ware tutorial documentation?", "Omithus", MB_ICONINFORMATION | MB_YESNO) == IDYES) {
			tutorial();
...
DWORD WINAPI input(PVOID lvpParameter)
{
	...
	while (hPipe != INVALID_HANDLE_VALUE)
	{
		if (ConnectNamedPipe(hPipe, NULL) != FALSE)
		{
			...
			execute_script(rL, L, WholeScript);
...
std::string WindoNamHojhdkjf = random_string(5);

LPCSTR g_szClassName = WindoNamHojhdkjf.c_str();
...
std::string scriptresponce;
bool whitelisted = false;

#define ExecuteButtonClicked 643
#define FreeButtonClicked 34234
#define LOAD_BUTTON 3343
#define ExplorerButton 1000
#define AsshurtESP 1001
#define WingsFlyDefine 1002
#define Bong_Script 1003

#pragma comment(linker,"\"/manifestdependency:type='win32' name='Microsoft.Windows.Common-Controls' version='6.0.0.0' processorArchitecture='*' publicKeyToken='6595b64144ccf1df' language='*'\"")

void ExecuteScriptFinal() {
	int len = SendMessage(CommandBar, WM_GETTEXTLENGTH, 0, 0);
	char *buf = new char[len + 1]();
	SendMessage(CommandBar, WM_GETTEXT, (WPARAM)len + 1, (LPARAM)(buf));

	std::string lawl = buf;

	execute_script(rL, L, lawl.c_str());
	delete[] buf;
}

void ExecuteGUICode() {
	std::string lawl = scriptresponce;
	execute_script(rL, L, lawl.c_str());
}

std::string readcontents(std::string pathway) {
	std::ifstream inFile;
	inFile.open(pathway);//open the input file
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CTLCOLORSTATIC:
	{
		...
		SetTextColor(hEdit, RGB(0, 0, 0)); // text color of console
		SetBkColor(hEdit, RGB(255, 255, 255)); // dont fuck with this, looks ugly on anything else but white

		if (CONSOLEREDTAG == (HWND)lParam) {
			SetBkColor(hEdit, RGB(215, 12, 32)); // dont fuck with this, looks ugly on anything else but white
			SetTextColor(hEdit, RGB(255, 255, 255)); // text color of console
		}
	...
	case WM_COMMAND:
		...
		{
		case ExecuteButtonClicked:
			ExecuteScriptFinal();
			break;
		case FreeButtonClicked:
			SetWindowText(CommandBar, "");
			break;
		case LOAD_BUTTON:
			try {
				...
				ofn.lpstrFilter = "LUA Files\0*.lua\0Text Files\0*.txt\0";
				ofn.lpstrFile = filename;
				ofn.nMaxFile = MAX_PATH;
				ofn.lpstrTitle = "Select a lua file to open!";
				ofn.Flags = OFN_DONTADDTORECENT | OFN_FILEMUSTEXIST;

				if (GetOpenFileNameA(&ofn))
				{
					SetWindowText(CommandBar, readcontents(filename).c_str());
				}
			}
			catch (...) {};
			break;
...
BOOL CreateLoginWindow()
{
	...
	//UnregisterClass(g_szClassName, hInstance2);
	...
	if (!LoadLibraryA("Msftedit.dll")) {
		MessageBoxA(0, "Failed to Load Msftedit.dll", "WARN", MB_OK);
	}

	hwnd = CreateWindowEx(WS_EX_TOPMOST, g_szClassName, WindoNamHojhdkjf.c_str(), WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 377, 372, 0, 0, hInstance2, 0);
	//center window
	...
	//CommandBar is defined at top so text can be retrieved on buttonclick.I seperated this because it's used and the others are fancy windows.
	CommandBar = CreateWindowEx(WS_EX_CLIENTEDGE, RICHEDIT_CLASS, "", WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | ES_MULTILINE | ES_AUTOVSCROLL | ES_DISABLENOSCROLL, 27, 70, 312, 236, hwnd, (HMENU)NULL, hInstance2, 0);
	//other windows
	HWND ExeButton = CreateWindowEx(NULL, "BUTTON", "EXECUTE", WS_CHILD | WS_VISIBLE, 26, 312, 114, 23, hwnd, (HMENU)ExecuteButtonClicked, hInstance2, 0);
	HWND ClearButton = CreateWindowEx(NULL, "BUTTON", "CLEAR", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 144, 312, 78, 23, hwnd, (HMENU)FreeButtonClicked, hInstance2, NULL);
	HWND LoadButton = CreateWindowEx(NULL, "BUTTON", "LOAD", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 227, 312, 113, 23, hwnd, (HMENU)LOAD_BUTTON, hInstance2, NULL);
	CONSOLEREDTAG = CreateWindowEx(NULL, "STATIC", "                                                                                                            ", WS_VISIBLE | WS_CHILD | WS_GROUP | SS_LEFT, 0, 2, 374, 15, hwnd, (HMENU)NULL, hInstance2, 0);

	//HWND Asshurt7Logo = CreateWindowEx(0, "STATIC", "Static", WS_VISIBLE | WS_CHILD | WS_GROUP | SS_LEFT | SS_CENTERIMAGE | SS_BITMAP, 497, 11, 210, 189, hwnd, (HMENU)0, hInstance2, 0);
	//HANDLE hImage = LoadImage(GetModuleHandle("SirHurt.dll"), MAKEINTRESOURCE(IDB_BITMAP1), IMAGE_BITMAP, 0, 0, NULL);

	//SendMessage(Asshurt7Logo, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hImage);

	HWND PushScript1 = CreateWindowEx(NULL, "BUTTON", "Explorer", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 110, 26, 71, 23, hwnd, (HMENU)ExplorerButton, hInstance2, NULL);
	HWND PushScript2 = CreateWindowEx(NULL, "BUTTON", "Asshurt ESP", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 255, 26, 84, 23, hwnd, (HMENU)AsshurtESP, hInstance2, NULL);
	HWND PushScript3 = CreateWindowEx(NULL, "BUTTON", "RemoteSpy", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 27, 26, 80, 23, hwnd, (HMENU)WingsFlyDefine, hInstance2, NULL);
	HWND PushScript4 = CreateWindowEx(NULL, "BUTTON", "Script Hub", WS_TABSTOP | WS_VISIBLE | WS_CHILD, 183, 26, 69, 23, hwnd, (HMENU)Bong_Script, hInstance2, NULL);

	HWND asshurturl = CreateWindowEx(0, "STATIC", "www.asshurthosting.pw", WS_VISIBLE | WS_CHILD | WS_GROUP | SS_LEFT | ES_CENTER, 26, 50, 314, 15, hwnd, (HMENU)NULL, hInstance2, 0);

	//set text limit for STATIC
	HFONT textFont = CreateFont(16, 0, 0, 0, FW_LIGHT, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Arial"));
	SendMessage(CommandBar, EM_LIMITTEXT, 0x7FFFFFFE, 0);

	//set text font
	SendMessage(CommandBar, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(ExeButton, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(ClearButton, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(LoadButton, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(PushScript1, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(PushScript2, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(PushScript3, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(PushScript4, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));
	SendMessage(asshurturl, WM_SETFONT, (WPARAM)textFont, MAKELPARAM(TRUE, 0));

	RemoveStyle(CommandBar, WS_EX_CLIENTEDGE);
...
static int httpget_impl(int rL) {
	printf("httpget\n");
	std::string URL = r_lua_tolstring(rL, -1, 0);
	std::string Ret = download_url(URL.c_str());
	execute_script(rL, L, Ret.c_str());
	printf("finished\n");
	return 1;
}
...
DWORD WINAPI main() {
	...
	try {
		...
		CreateConsole((PCHAR)"Ashpe Debug Console");
		...
		script_context = Memory::Scan((char*)&vf_table);

		if (debug_mode)
			output_label("Omithus: Initializing lua state");
		...
		r_lua_pushnumber(rL, (double)(DWORD)httpget_impl);
		r_lua_pushcclosure(rL, hook, 1);
		...
		MessageBox(NULL, "Exception handled during main scan. Please try again.", "Ashpe", MB_ICONERROR | MB_OK);
		exit(0);
	}
